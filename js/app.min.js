function testWebP(callback) {
    ...
    if (isMobile.any()) {
        ...
        if (menuArrows.length > 0) {
            for (let i = 0; i < menuArrows.length; i++) {
                ...
                menuArrow.addEventListener('click', function(e) { menuArrow.parentElement.classList.toggle('_active') })
            }
        }
    ...
    if (isIeEdge) {
        function ibg() {
            ...
            for (let i = 0; i < ibgmages.length; i++) { ibgmages[i].classList.add('ibg') }
    ...
    searchHeaderClose.addEventListener('click', function() { searchHeader.classList.remove('_active') });
    searchHeader.addEventListener('click', function(e) { if (e.target == searchHeader) { searchHeader.classList.remove('_active') } });
    "use strict";
    ...
    DynamicAdapt.prototype.init = function() {
        ...
        this.mediaQueries = Array.prototype.map.call(this.оbjects, function(item) { return '(' + this.type + "-width:" + item.breakpoint + "px)," + item.breakpoint }, this);
        this.mediaQueries = Array.prototype.filter.call(this.mediaQueries, function(item, index, self) { return Array.prototype.indexOf.call(self, item) === index });
        for (let i = 0; i < this.mediaQueries.length; i++) {
            const media = this.mediaQueries[i];
            const mediaSplit = String.prototype.split.call(media, ',');
            const matchMedia = window.matchMedia(mediaSplit[0]);
            const mediaBreakpoint = mediaSplit[1];
            const оbjectsFilter = Array.prototype.filter.call(this.оbjects, function(item) { return item.breakpoint === mediaBreakpoint });
            matchMedia.addListener(function() { _this.mediaHandler(matchMedia, оbjectsFilter) });
    ...
    DynamicAdapt.prototype.mediaHandler = function(matchMedia, оbjects) {
        ...
        } else { for (let i = 0; i < оbjects.length; i++) { const оbject = оbjects[i]; if (оbject.element.classList.contains(this.daClassname)) { this.moveBack(оbject.parent, оbject.element, оbject.index) } } }
    };
    DynamicAdapt.prototype.moveTo = function(place, element, destination) {
        element.classList.add(this.daClassname);
        if (place === 'last' || place >= destination.children.length) { destination.insertAdjacentElement('beforeend', element); return }
    ...
    DynamicAdapt.prototype.indexInParent = function(parent, element) { const array = Array.prototype.slice.call(parent.children); return Array.prototype.indexOf.call(array, element) };
    ...
    steppers.forEach(function(stepper) {
        ...
        stepperInput.addEventListener('keyup', (e) => {
            ...
            if (count > parseInt(stepperInput.dataset.max)) { self.value = self.dataset.max }
        ...
        stepperInput.addEventListener('keypress', (e) => { allowNumbersOnly(e) });
        ...
        stepperBtnUp.addEventListener('click', (e) => {
            e.preventDefault();
            count++;
            if (count > parseInt(stepperInput.dataset.max)) { count = parseInt(stepperInput.dataset.max) }
            if (count == 1) { stepperBtnDown.classList.add('stepper__btn_disabled') } else { stepperBtnDown.classList.remove('stepper__btn_disabled') }
            stepperInput.value = count;
            if (isNotApple) { stepperInput.style.width = `${stepperInput.value.length + 1}ex` } else { stepperInput.style.width = `${stepperInput.value.length + 2}ex` }
        });
    ...
    quantities.forEach(function(quantity) {
        ...
        quantityInput.addEventListener('keyup', (e) => {
            ...
            if (self.value == '0') { self.value = 1 }
            count = self.value;
            if (count > parseInt(quantityInput.dataset.max)) { self.value = self.dataset.max }
            if (isNotApple) { self.style.width = `${self.value.length + 1}ex` } else { self.style.width = `${self.value.length + 2}ex` }
        ...
        quantityInput.addEventListener('keypress', (e) => { allowNumbersOnly(e) });
        ...
        quantityBtnUp.addEventListener('click', (e) => {
            e.preventDefault();
            count++;
            if (count > parseInt(quantityInput.dataset.max)) { count = parseInt(quantityInput.dataset.max) }
            if (count == 1) { quantityBtnDown.classList.add('quantity__btn_disabled') } else { quantityBtnDown.classList.remove('quantity__btn_disabled') }
            quantityInput.value = count;
            if (isNotApple) { quantityInput.style.width = `${quantityInput.value.length + 1}ex` } else { quantityInput.style.width = `${quantityInput.value.length + 2}ex` }
        });
    ...
    function testWebP(callback) {
        ...
        webP.src = "data:image/webp;base64,UklGRjoAAABXRUJQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA}
        testWebP(function(support) { if (support == !0) { document.querySelector('body').classList.add('webp') } else { document.querySelector('body').classList.add('no-webp') } });
        $clamp(document.querySelector('.popular-posts__title'), { clamp: 1, useNativeClamp: !1 });
